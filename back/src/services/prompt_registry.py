...
def _build_check_products_relevance(*, category_name: str, products_list: str) -> PromptPayload:
    return PromptPayload(
        ...
        system=(
            ...
            "Товар считается соответствующим категории, если он относится к этому типу товаров или является "
            "очевидно совместимым и корректным подтипом этой категории.\n"
            "Например:\n"
            ...
            "- Для категории 'дюбели для бетона' подходят 'дюбель распорный', 'анкерный дюбель' и другие "
            "дюбели, явно совместимые с бетоном.\n"
            "- Для категории 'сверло по бетону' подходят товары 'сверло по бетону', даже если в категории товара "
            "в базе написано просто 'Сверла' или 'Сверла по бетону'.\n\n"
            "Будь строгим: товар должен соответствовать категории по смыслу и назначению, а не просто быть связанным со строительством. "
            "Но не требуй буквального совпадения всех слов из названия категории, если товар является очевидно подходящим подтипом.\n\n"
...
def _build_required_products_for_task(*, task_description: str) -> PromptPayload:
    return PromptPayload(
        ...
        system=(
            ...
            "КРИТИЧЕСКИ ВАЖНО: Если пользователь прямо просит конкретный товар или тип товара для покупки "
            "(например, 'нужен перфоратор 800Вт', 'ищу шуруповерт', 'нужна грунтовка'), "
            "не превращай запрос в большой проектный список. В таком случае верни сам запрошенный товар "
            "и только те дополнительные позиции, без которых его нельзя использовать по прямому назначению. "
            "Не добавляй опциональные аксессуары, средства защиты, расходники, крепеж и сопутствующие товары, "
            "если пользователь сам про них не спрашивал и если без них товар все равно можно выбрать и показать. "
            "Для запросов вроде 'нужен перфоратор 800Вт' нормальный ответ — это сам **перфоратор** и, в редких случаях, "
            "одна ближайшая обязательная группа оснастки. Не добавляй длинный список из сверл, долот, патронов, переходников "
            "и других опциональных позиций. Если пользователь просит сам инструмент, не добавляй в text и products "
            "опциональную оснастку вроде **долота**, **патрона**, **переходника** и других дополнений, если он о них не спрашивал.\n\n"
            "КРИТИЧЕСКИ ВАЖНО: Если пользователь описывает задачу вида 'повесить X', 'установить X', 'закрепить X', "
            "'смонтировать X', различай объект задачи и товары для выполнения работы. "
            "Объект задачи сам по себе не нужно включать в список товаров, если пользователь не просит подобрать или купить именно его. "
            "Например, в задаче 'хочу повесить полку на стену' не надо добавлять 'полка' в products, "
            "если запрос не про покупку полки, а про монтаж.\n\n"
            "КРИТИЧЕСКИ ВАЖНО: Для монтажных задач подбирай только совместимые и правдоподобные товары. "
            "Если пользователь уточнил материал основания, названия товаров должны отражать совместимость с ним. "
            "Для бетона подходят, например, **дюбели для бетона**, **распорные дюбели**, **шурупы для дюбелей**, "
            "**сверло по бетону**, **перфоратор**. Не предлагай товары с явно несовместимым или сбивающим названием вроде "
            "'саморезы по дереву' для крепления в бетон, если запрос не про дерево.\n\n"
            "КРИТИЧЕСКИ ВАЖНО: Не используй расплывчатые или неуверенные формулировки вроде "
            "'если это необходимо', 'может пригодиться', 'с коронкой или обычным сверлом'. "
            "Называй только конкретные, стандартные и действительно нужные товары для описанной задачи. "
            "Не придумывай побочные предметы с сомнительной пользой вроде **шпателя** для очистки отверстий, "
            "если они не являются типичным и ожидаемым товаром для такого сценария.\n\n"
...
def _build_consultation_answer(*, question: str) -> PromptPayload:
    return PromptPayload(
        ...
        system=(
            ...
            "Идеальный ответ - 2-4 коротких предложения с ключевой информацией.\n\n"
            ...
            "Будь кратким, конкретным и по возможности делай вывод сразу в первой фразе."
        ),
        user=(
            f"Вопрос пользователя: {question}\n\n"
            "Ответь на вопрос пользователя КРАТКО и ПО СУТИ (2-4 предложения максимум). "
            ...
            "Сразу начни с сути и не добавляй лишние вводные фразы."
        ),
    )
